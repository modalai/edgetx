...
local scrollStart = getTime()
local scrollField = nil
local scrollVal   = nil
  ...
  repeat
    ...
    if not last then
      ...
      elseif b ~= 0 then
        ...
        opt = opt .. (CHAR_SUBST[b] or string.char(b))
      end
    end
  until b == 0
...
local function dynCol(name)
  if not name then return COL2 end
  local nameW = lcd.sizeText and lcd.sizeText(name) or (#name * (textSize < 10 and 6 or 10))
  return math.min(COL2, COL1 + nameW + 4)
end
...
local function fieldTextSelDisplay_bw(field, y, attr)
  local x = dynCol(field.name)
  local val = field.values[field.value+1] or "ERR"
  local charW = textSize < 10 and 6 or 10
  local maxChars = math.ceil((LCD_W - x) / charW)
  if #val > maxChars then
    local offset = 0
    if attr ~= 0 and not edit then
      -- only scroll when highlighted but not in edit/selection mode
      if scrollField ~= field or scrollVal ~= val then
        scrollField = field
        scrollVal   = val
        scrollStart = getTime()
      end
      local overflow = #val - maxChars
      local scrollStep = 080 -- ticks per character (~800ms each)
      local pauseStart = 100 -- ticks at start
      local pauseEnd   = 300 -- ticks at end (~3s) before restart
      local cycleTicks = pauseStart + overflow * scrollStep + pauseEnd
      local t = (getTime() - scrollStart) % cycleTicks
      if t >= pauseStart and t < pauseStart + overflow * scrollStep then
        offset = math.floor((t - pauseStart) / scrollStep)
      elseif t >= pauseStart + overflow * scrollStep then
        offset = overflow
      end
    else
      -- clear so timer resets next time this field is highlighted
      if scrollField == field then scrollField = nil scrollVal = nil end
    end
    val = string.sub(val, offset + 1, offset + maxChars)
  end
...
local function fieldStringDisplay(field, y, attr)
  lcd.drawText(dynCol(field.name), y, field.value, attr)
end
